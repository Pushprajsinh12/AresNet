...
def show_spinner(stop_event, message="Running Nmap..."):
...
def run_nmap_scan(target, ports=None, use_sudo=True, output_file=None, export_json=False, export_csv=False, use_pn=False, script=None):
    ...
    try:
        write_output("[*] Running Nmap...", output_file)
        ...
        if not nmap_xml.strip():
            write_output("[!] Nmap returned empty result. Check host status or try --nmap-noroot", output_file)
            return []

        write_output("\n--- Nmap Scan Output ---", output_file)
        ...
        write_output(f"[ERROR] Nmap scan failed: {e}", output_file)
        return []
...
def main():
    ...
    parser.add_argument('-T', '--timing', default='T3')
    ...
    parser.add_argument('--nmap', action='store_true', help='Use Nmap scan (requires sudo for full scan)')
    parser.add_argument('--nmap-noroot', action='store_true', help='Run Nmap without sudo using -sT')
    parser.add_argument('--nmap-pn', action='store_true', help='Skip host discovery (treat host as online)')
    ...
    if args.discover:
        ...
        def tcp_task():
            ...
            tcp_results = scan_ports(
                args.target, args.ports, args.threads, grab_banner=args.banner,
        ...
        write_output(f"\n[*] TCP scan completed. Found {len(tcp_results)} results.", args.output)
        ...
        if args.nmap:
            nmap_results = run_nmap_scan(
                args.target,
                use_sudo=not args.nmap_noroot,
                output_file=args.output,
                export_json=False,
                export_csv=False,
                use_pn=args.nmap_pn,
                script=args.nmap_script
            )
